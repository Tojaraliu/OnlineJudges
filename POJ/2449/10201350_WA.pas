...
begin
  t:=x;
  if ((a[x].g+a[x].h)>(a[x<<1].g+a[x<<1].h))and(x<<1<=size) then t:=x<<1;
  if ((a[t].g+a[t].h)>(a[t+1].g+a[t+1].h))and(t+1<=size) then inc(t);
  if t<>x then begin swap(a[t],a[x]);down(t);end;
end;
...
Var
  pq:heap;
  e,re:Array[0..20020] of point;
  ee:Array[0..20000000] of link;
  g,q,times:Array[0..20020] of int;
  v:Array[0..20020] of boolean;
  n,m,st,fn,kk,ne:int;
